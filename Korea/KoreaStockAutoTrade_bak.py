...
# 종목별 날자별, 13일 이상 경과한 경우 매도 후 해당 레코드 삭제하는 함수
def process_and_sell_first_record(sym):
    ...
    if sym in bought_stock_dates and len(bought_stock_dates[sym]) > 0:
        stock_name = get_stock_name(sym)
        current_price = get_current_price(sym)  # 현재가 가져오기
        ma20 = get_moving_average(sym, 20)      # 10일 이동평균선
        # 각 SEQ 레코드 확인
        for record in bought_stock_dates[sym][:]:  # 리스트 복사본 사용 (리스트 수정 중 반복)
            ...
            # 매수 후 13일 이상 경과 및 수익률이 플러스일 경우 매도
            if days_held >= 13 and current_price > buy_price:
                send_message(f"{sym}({stock_name})({buy_qty}주)(가격:{buy_price}) 매수 후 {days_held}일 경과되어 자동 매도 진행.")
                sell(sym, buy_qty)  # 매도 로직
                time.sleep(1)
                # 기록 삭제
...
try:
    ...
    send_message("===국내 주식 이평선돌파 자동매매 프로그램 시작===")
    send_message("-----------------------------------------------")
        ...
        if t_start < t_now < t_buy :  # AM 09:30 ~ PM 03:00 : 손절 체크
            if t_now.minute == 10 or t_now.minute == 40: 
                ...
                for sym in symbol_list:
                    ...
                    if ((prev_close_price < ma10_prev and current_price > ma10) or (prev_close_price < ma20_prev and current_price > ma20)) and current_price * mv10 > 1000000000:
                        send_message(f"{sym}({stock_name}) 10일 or 이평 돌파 매수 대상. 현재가:{current_price}, 10일: {ma10}, 이평: {ma20}, 종가(직전): {prev_close_price}, 10일(직전):{ma10_prev}, 20일(직전):{ma20_prev}")
            ...
            for sym in symbol_list:
                if len(stock_dict) < target_buy_count:
                    ...
                    if ((prev_close_price < ma10_prev and current_price > ma10) or (prev_close_price < ma20_prev and current_price > ma20) ) and current_price * mv10 > 1000000000:
                        ...
                        if buy_qty > 0:
                            send_message(f"{sym}({stock_name}) 10억이상, 10일 or 이평 돌파 ({buy_qty})개 매수 시도.")
        ...
        if t_sell < t_now < t_exit:  # PM 03:15 ~ PM 03:20 : 일괄 매도
            if soldout == False:
                ...
                for sym in stock_dict.keys():
                    ...
                    # 종목 매수 후 첫 번째 기록 가져오기 및 13일 경과 시 매도
