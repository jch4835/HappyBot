...
with open('C:\\git\\HappyBot\\Korea_js\\config.yaml', encoding='UTF-8') as f:    
...
def load_bought_stock_dates():
    try:
        with open('C:\\git\\HappyBot\\Korea_js\\bought_stock_dates.json', 'r') as f:
            return json.load(f)
...
def save_bought_stock_dates(bought_stock_dates):
    with open('C:\\git\\HappyBot\\Korea_js\\bought_stock_dates.json', 'w') as f:
...
def get_buy_amount(symbol: str) -> int:
    ...
    BUY_AMOUNT_MAP = {
        "465580": 400_000,   # ACE 미국빅테크TOP7 Plus
        "381180": 400_000,   # TIGER 미국필라델피아반도체나스닥
        "457480": 400_000,   # ACE 테슬라벨류체인액티브
        "438080": 800_000,   # ACE 미국S&P500미국채혼합50액티브
        "438100": 800_000,   # ACE 미국나스닥100미국채혼합50액티브        
    }

    DEFAULT_BUY_AMOUNT = 400_000  # 설정 없는 종목 기본금액
