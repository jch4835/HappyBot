...
def get_bollinger_band(market="NAS", code="NVDA", period=20):
    """블린저밴드 상/하단 계산"""
    PATH = "uapi/overseas-price/v1/quotations/dailyprice"
    URL = f"{URL_BASE}/{PATH}"
    headers = {
    ...
    closes = [float(item['clos']) for item in data[:period]]
    prev_closes = [float(item['clos']) for item in data[1:period+1]]
    ...
    prev_mean = sum(prev_closes) / len(prev_closes)
    prev_std = statistics.stdev(prev_closes)
    prev_lower = prev_mean - (2 * prev_std)
    prev_upper = prev_mean + (2 * prev_std)
...
def process_and_sell_first_record(sym):
    ...
    if sym in bought_stock_dates and len(bought_stock_dates[sym]) > 0:
        ...
        current_price = get_current_price(market2, sym)
        lower, upper, prev_lower, prev_upper = get_bollinger_band(market2, sym)
        ...
        for record in bought_stock_dates[sym][:]:
            ...
            if is_december and bb_upper_cross:
                ...
                sell(market=market1, code=sym, qty=buy_qty, price=current_price)
                time.sleep(1)
...
try:
    ...
    print(ACCESS_TOKEN)
    # ACCESS_TOKEN="eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzUxMiJ9.eyJzdWIiOiJ0b2tlbiIsImF1ZCI6IjZjNzhlYzBhLWFhZjEtNGU2Ny05NGNmLTdhNjVhODIxM2RiNyIsInByZHRfY2QiOiIiLCJpc3MiOiJ1bm9ndyIsImV4cCI6MTc3MTkxNTc4MCwiaWF0IjoxNzcxODI5MzgwLCJqdGkiOiJQU0p0VkVOVjZjYXRwdVo2dVJ4RWxySHNnVmI1RkhYWVNoaXAifQ.xW0zLfj7uGh8ju19SC-AoSsa8ifXIkbXAzuhVwPsTqR9jhu0wXjqvO5DeNQnWexwIEEeI6vkDprMH8GKg0jTLA"
    nasd_symbol_list = ["TQQQ"]
    nyse_symbol_list = [] 
    amex_symbol_list = [] # 
    symbol_list = nasd_symbol_list + nyse_symbol_list + amex_symbol_list
    send_message("===해외 주식 BB하단돌파 자동매매 프로그램을 시작합니다===")
    send_message("--------------------------------------------")
    ...
    while True:
        ...
        t_buy = t_now_ny.replace(hour=1, minute=30, second=0, microsecond=0)
        t_sell = t_now_ny.replace(hour=1, minute=30, second=0, microsecond=0)
        t_exit = t_now_ny.replace(hour=16, minute=39, second=0, microsecond=0)
        ...
        if t_start < t_now_ny < t_buy :  # AM 9:30 ~ PM 03:30 : 모니터링
            if t_now_ny.minute % 2 == 0: 
                ...
                for sym in symbol_list:
                    ...
                    lower, upper, prev_lower, prev_upper = get_bollinger_band(market2, sym)
                    print("lower:",lower)
                    print("prev_lower:",prev_lower)
                    print("upper:",upper)
                    print("prev_upper:",prev_upper)
                    print("prev_close_price:",prev_close_price)
                    print("ma10_prev:",ma10_prev)
                    print("ma20_prev:",ma20_prev)
                    ...
                    if prev_close_price < ma10_prev and prev_close_price < ma20_prev and prev_close_price < prev_lower and current_price > lower:
                        send_message(f"{sym}({stock_name}) BB하단 골드크로스 매수 대상. 현재가:{current_price}, lower: {lower}, prev_lower: {prev_lower}, 종가(직전): {prev_close_price}, 10일(직전):{ma10_prev}, 이평(직전):{ma20_prev}")
                time.sleep(1)
            ...
            for sym in symbol_list:
                if len(stock_dict) < target_buy_count:
                    ...
                    if prev_close_price < ma10_prev and prev_close_price < ma20_prev and prev_close_price < prev_lower and current_price > lower:
                        ...
                        if buy_qty > 0:
                            send_message(f"{sym}({stock_name}) BB하단 골드크로스 돌파하여  ({buy_qty})개 매수를 시도합니다. 현재가:{current_price}, lower: {lower}, prev_lower: {prev_lower}, 종가(직전): {prev_close_price}, 10일(직전):{ma10_prev}, 이평(직전):{ma20_prev} ")
                            market = "NASD"
                            if sym in nyse_symbol_list:
                                market = "NYSE"
                            if sym in amex_symbol_list:
