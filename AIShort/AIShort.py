...
print(f"초기 자산: {START_CASH:,.0f}")
print(f"최종 자산: {final_asset:,.0f}")
print(f"총 수익률: {total_return*100:.2f}%")
# print(f"MDD: {mdd*100:.2f}%")
print(f"승률: {win_rate*100:.2f}%")
print(f"총 거래 수: {wins+losses}")

print("\n📌 ===== 전체 거래 로그 =====")

for t in trade_log:
    if t['type'] == "BUY":
        print(f"[매수] {t['date'].date()} {t['symbol']}({get_stock_name(t['symbol'])}) | 가격:{t['price']:.0f} | 수량:{t['qty']} | {t['reason']}")
    else:
        print(f"[매도] {t['date'].date()} {t['symbol']}({get_stock_name(t['symbol'])}) | 가격:{t['price']:.0f} | 수량:{t['qty']} | 수익률:{t['return']*100:.2f}% | {t['reason']}")
...
#     plt.title("Equity Curve (Real Portfolio)")
#     # plt.show()
# except:
#     pass